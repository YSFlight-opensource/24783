...
protected:
	...
	int targetState;
	int targetX,targetY,targetWid,targetHei;
...
void FsLazyWindowApplication::MoveTarget(void)
{
	int wid,hei;
...
void FsLazyWindowApplication::CheckHitTarget(void)
{
	int wid,hei;
	FsGetWindowSize(wid,hei);

	if(targetX<=missileX && missileX<targetX+targetWid &&
	   targetY<=missileY && missileY<targetY+targetHei)
	{
		targetX=0;
		targetWid-=10;
		if(targetWid<30)
...
// 		}
		...
		{
			targetX          ,targetY,
			targetX+targetWid,targetY,
			targetX+targetWid,targetY+targetHei,
			targetX          ,targetY+targetHei,
		};
		renderer.DrawVtx(GL_TRIANGLE_FAN,4,targetVtx);
	}
