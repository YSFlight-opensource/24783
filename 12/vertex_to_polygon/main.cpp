...
public:
	...
	void VtxNomToYsShell(YsShellExt &shl,const std::vector <float> &vtx,const std::vector <float> &nom,const std::vector <float> &col);
...
void FsLazyWindowApplication::LoadBinaryStl(const char fn[])
{
	...
	VtxNomToYsShell(shl,vtx,nom,col);
	CacheBoundingBox();
}
...
void FsLazyWindowApplication::VtxNomToYsShell(YsShellExt &shl,const std::vector <float> &vtx,const std::vector <float> &nom,const std::vector <float> &col)
{
	...
	for(int i=0; i<vtx.size()/9; ++i)
	{
		...
		YsColor plCol;
		plCol.SetIntRGBA(col[i*12],col[i*12+1],col[i*12+2],col[i*12+3]);
		shl.SetPolygonColor(plHd,plCol);
	}

	vtxToPlg.SetShell(shl);
}
	...
	for(auto plHd : shl.AllPolygon())
	{
		...
		if(3<=plVtHd.GetN())
		{
			...
			for(auto vtHd : plVtHd)
			{
				...
				col.push_back(plCol.Ri());
				col.push_back(plCol.Gi());
				col.push_back(plCol.Bi());
				col.push_back(1);
			}
		}
	}
...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	{
		auto vtHd=PickedVertex(mx,my);
		if(nullptr!=vtHd)
		{
			auto plgPtr=vtxToPlg.FindPolygonFromVertex(shl,vtHd);
			if(nullptr!=plgPtr)
			{
				for(auto plHd : *plgPtr)
				{
