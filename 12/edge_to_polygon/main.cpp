...
protected:
	...
	YsShellExt shl;
	EdgeToPolygonTable edgeToPolygon;
...
public:
	...
	static void VtxNomColToYsShell(YsShellExt &shl,const std::vector <float> &vtx,const std::vector <float> &nom,const std::vector <float> &col);
...
void FsLazyWindowApplication::LoadBinaryStl(const char fn[])
{
	...
	VtxNomColToYsShell(shl,vtx,nom,col);
	CacheBoundingBox();
	edgeToPolygon.CleanUp();
	edgeToPolygon.SetShell(shl);
}
...
/* static */ void FsLazyWindowApplication::VtxNomColToYsShell(YsShellExt &shl,const std::vector <float> &vtx,const std::vector <float> &nom,const std::vector <float> &col)
{
	...
	for(int i=0; i<vtx.size()/9; ++i)
	{
		...
		YsColor plCol;
		plCol.SetDoubleRGBA(col[i*12],col[i*12+1],col[i*12+2],col[i*12+3]);
...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	if(evt==FSMOUSEEVENT_LBUTTONDOWN)
	{
		...
		if(nullptr!=plHd)
		{
			...
			{
				auto edPlPtr=edgeToPolygon.FindPolygonFromEdge(shl,plVtHd[edIdx],plVtHd.GetCyclic(edIdx+1));
				if(nullptr!=edPlPtr)
				{
					for(auto plHd : *edPlPtr)
					{
